...
CREATE_VEC_AND_ARRAY(sharedDM, zLocalVec, zGlobalVec, zLocalArray);
  ...
  PetscPrintf(PETSC_COMM_WORLD, "fields initialized at time %g\n", stagetime);
...
if (process->addtoRHS){*rhophiSource += rhog* *diva;}
  ...
  for (PetscInt i = cellRange.start; i < cellRange.end; ++i) {
    ...
    // const auto &fluxGrad = fluxGradValues[i - cellRange.start];
...
if (process->addtoRHS){*rhophiSource += *phik * (ux * Drhogx + uy * Drhogy + uz * Drhogz)*0;}
